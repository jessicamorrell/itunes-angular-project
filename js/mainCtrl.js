...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    //Code here
    // $scope.newArrayOfObjects = [];
    // $scope.newArrayMaker = function(arrOfObjs) {
    //   for (var i = 0; i < arrOfObjs.length; i++) {
    //     for(var key in arrOfObjs[i]) {
    //       AlbumArt = arrOfObjs[i].artworkUrl30[key];
    //       Artist = arrOfObjs[i].artistName;
    //       Collection = arrOfObjs[i].collectionName;
    //       CollectionPrice = arrOfObjs[i].collectionPrice;
    //       Play = arrOfObjs[i].previewUrl;
    //     }
    //   }
    //   $scope.newArrayOfObjects.push(arrOfObjs);
    // }


    $scope.getSongData = function(artist) {
      itunesService.retrieveSong(artist)
      .then(function(response) {
        $scope.songData = response.data.results
        var newArray = [];
        $scope.songData.forEach(function(element) {
          newArray.push({
          AlbumArt: element.artworkUrl100,
  ...
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the iTunes data pushing into your new array objects that look like the above data.
    ...
    //   AlbumArt = response.artworkUrl30,
    //   Artist = response.artistName,
    //   Collection = response.collectionName,
    //   CollectionPrice = response.collectionPrice,
    //   Play = response.previewUrl
    // }
    //$scope.newArrayOfObjects.push()
